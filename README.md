...
Платформа инженерных расчётов на прочность: калькуляторы, справочники и админ-панель.
...
1. Закройте все терминалы с `npm run dev` и процессы, использующие проект.
...
- [x] Справочник сталей ПНАЭ (Rm, Rp0,2, A, Z, α, E по температуре)
...
Справочники в БД, остальные калькуляторы, закладки, S3 для файлов, PDF-генерация на сервере.
