...
import { GostSteelLegend } from "@/components/handbooks/GostSteelLegend";
...
import { GostSteelMarkCatalog } from "@/components/handbooks/GostSteelMarkCatalog";
...
import type { GostAllowableByModeResult } from "@/lib/gost34233_1Allowables";
import { computeGostAllowableByMode } from "@/lib/gost34233_1Allowables";

export interface GostTableRow {
  id: string;
  categoryId: string;
  group: string;
  mark: string;
...
function rowTabLabel(row: GostTableRow, index: number): string {
...
type RowCalc = {
  row: GostTableRow;
  grade: ReturnType<typeof findGrade> | undefined;
  mech: Record<SteelPropertyKey, number | null>;
  allowable: GostAllowableSnapshot | null;
  ready: boolean;
};
...
function useRowCalcs(handbook: SteelHandbook, rows: GostTableRow[]): RowCalc[] {
  return useMemo(
    () =>
      rows.map((row) => {
        const grade =
          row.group && row.mark && row.sortament ? findGrade(handbook, row.group, row.mark, row.sortament) : undefined;
...
const TABLE_HEAD = "bg-[var(--color-primary)] text-[var(--color-primary-foreground)]";
  ...
  return (
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div className="min-w-0">
        ...
        {subtitle ? (
          <p className="mt-1 text-sm font-normal text-[var(--color-muted-foreground)] sm:text-base">{subtitle}</p>
        ) : null}
      </div>
      {action}
    </div>
...
function InlineColumnHeader({ children }: { children: ReactNode }) {
  return <div className={cn("mx-auto min-w-0 text-center font-serif", TABLE_HEAD_FONT)}>{children}</div>;
}
...
function MechTableColumnHeader({
  ...
  return (
    <InlineColumnHeader>
      <MechPropertyHeader propertyKey={propertyKey} unit={unit} atTemperature={atTemperature} />
    </InlineColumnHeader>
  );
}
...
function ResultsBlock({
  ...
  return (
    <div className="space-y-10">
        ...
        <CardContent className="space-y-6 p-4 sm:p-6">
          <SectionHeading
            action={
              ...
              >
                <input type="checkbox" checked={showExtraProps} onChange={(e) => onShowExtraPropsChange(e.target.checked)} />
                Показать α и Z
              </label>
            }
          >
              ...
              <thead>
                <tr className={TABLE_HEAD}>
                  ...
                  {mechCols.map((c, colIdx) => (
                    <th key={c.key} className={cn(TH_VALUE, colIdx === mechCols.length - 1 && "rounded-tr-xl")}>
                      <MechTableColumnHeader propertyKey={c.key as MechPropertyKey} unit={mechColumnUnit(c.key, handbook.properties[c.key]?.unit ?? "")} atTemperature />
                    </th>
                  ))}
                </tr>
              </thead>
      ...
      <Card className={RESULT_CARD}>
        <CardContent className="space-y-6 p-4 sm:p-6">
          <SectionHeading>Допускаемые напряжения</SectionHeading>
            ...
            {PNAE_ALLOWABLE_MODES.map((mode) => (
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    onAllowableModesChange(toggleAllowableMode(allowableModes, mode.id, e.target.checked))
                  }
                />
                {mode.shortLabel}
              </label>
              ...
              <tbody>
                {allowableRows.length === 0 ? (
                  <tr className="border-t border-[var(--color-border)]/50">
                    <td colSpan={6} className="px-5 py-8 text-center text-sm text-[var(--color-muted-foreground)]">
                      Нет данных для выбранных условий расчёта
                    </td>
                  </tr>
                ) : (
                  allowableRows.map(({ calc, def, result }) => (
                    <tr key={`${calc.row.id}-${def.id}`} className="border-t border-[var(--color-border)]/50">
                      <td className={TD_MARK}>
                        <MarkCell mark={calc.row.mark} sortament={calc.row.sortament} prominent />
                      </td>
                      <td className={cn(TD_BASE, "pnae-handbook-table-td py-6 font-medium text-[var(--color-foreground)]")} title={def.fullLabel}>
                        {def.shortLabel}
                      </td>
                      <td className={TD_BASE}>
                      ...
                      <td className={TD_VALUE}>
                        {result.sigma != null ? fmt(result.sigma) : "—"}
                      </td>
                      <td className={TD_VALUE}>{result.sigma13 != null ? fmt(result.sigma13) : "—"}</td>
                      <td className={TD_VALUE}>
...
export function GostSteelWorktable({ handbook }: Props) {
  const [rows, setRows] = useState<GostTableRow[]>(() => [buildEmptyRow()]);
  const [activeRowId, setActiveRowId] = useState(() => rows[0]?.id ?? "");

  useEffect(() => {
    if (rows.length > 0 && !rows.some((r) => r.id === activeRowId)) {
      setActiveRowId(rows[0]?.id ?? "");
    }
  }, [rows, activeRowId]);

  const [allowableModes, setAllowableModes] = useState<PnaeAllowableMode[]>(DEFAULT_PNAE_ALLOWABLE_MODES);
  const [showExtraProps, setShowExtraProps] = useState(false);

  const onRowChange = useCallback((id: string, patch: Partial<GostTableRow>) => {
  ...
  const onDuplicateRow = useCallback((id: string) => {
    ...
    setRows((list) => {
      ...
      if (!src) return list;
      const copy: GostTableRow = { ...src, id: newId };
  ...
  const selectMark = useCallback(
    (mark: string) => {
      ...
      if (!mark) {
        onRowChange(activeRow.id, { mark: "", group: "", sortament: "" });
        return;
      }
  ...
  return (
    <div className="pnae-section-gap min-w-0">
      ...
      <section>{/* ГОСТ легенда (с тем же набором обозначений σ/σ13/σRV) */}</section>
      <section>
        <GostSteelLegend />
      </section>
    </div>
...
export function buildEmptyRow(): GostTableRow {
  return {
    id: newRowId(),
    categoryId: "",
    group: "",
